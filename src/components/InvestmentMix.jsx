import { investmentMix } from "../data/benchmarkData";
import SectionWrapper from "./SectionWrapper";

export default function InvestmentMix() {
  const { title, subtitle, intro, columns, rows, note, callout } = investmentMix;
      ...
      <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
        ...
        <div className="overflow-x-auto rounded-xl shadow-md">
          <table className="w-full min-w-[720px] text-sm">
            ...
            <tbody>
              {rows.map((row, i) => (
                <tr key={row.category} className={i % 2 === 0 ? "bg-white" : "bg-culligan-off-white"}>
                  <td className="px-4 py-4 font-semibold text-culligan-deep">{row.category}</td>
                  <td className="px-4 py-4 text-culligan-red font-medium">{row.culligan}</td>
                  <td className="px-4 py-4 text-culligan-muted">{row.peer}</td>
                  <td className="px-4 py-4 text-culligan-amber font-medium">{row.gap}</td>
                  <td className="px-4 py-4 text-culligan-body">{row.implication}</td>
                </tr>
              ))}
            </tbody>
          </table>
